...
Template.f_imagesCustom.helpers({
  'tData': function(){
    const tpl = Template.instance();
    return tpl.tData.get();
  }
})
Template.f_imagesCustom.onCreated(function(){
  this.tData = new ReactiveVar([]);
  this.data.reload = new ReactiveVar();
})
Template.f_imagesCustom.onRendered(function(){
  const method = BaseTemplate.instances[this.data.name].method+'/GetImages';
  const self = this;
...
Template.f_imagesCustom.events({
  'click #remove': function(e, tpl){
    e.preventDefault();
    Session.set('isLoadingScreen', true);
    const { key } = e.currentTarget.dataset;
    const method = BaseTemplate.instances['f_fraud'].method+'/RemoveImage';
    dsLog.info('[CALL] '+method);
    Meteor.call(method, {hash: key}, function(error, result){
