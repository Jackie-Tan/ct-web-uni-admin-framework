  ...
  function () {
    ...
    for (let order in listCate) {
      ...
      if (bigCate == category) {
        categories.push({
          "value": category,
          "display": `--${Bconf.getS(`cat.${category}.name.vi`)}--`,
          "is_disable": true,
          "css": "background-color:#dcdcc3;font-weight:bold;"
        });
        continue
      }
      categories.push({
    ...
    for (let order in listRegion) {
      ...
      regions.push({
        "value": region,
        "display": `${Bconf.getS(`common.region.${region}.name.vi`)}`
      })
      areas[region] = [{
        "value": "",
        "display": "vui lòng chọn quận/huyên"
      }];
    ...
    let regions_v2 = [{
      "value": "",
      "display": "vui lòng chọn tỉnh/thành phố"
    }];
    let areas_v2 = {};
    let wards = {}
    ...
    for (let order in listRegionV2) {
      ...
      regions_v2.push({
        "value": region,
        "display": `${Bconf.getS(`common.1.region_v2.${region}.name.vi`)}`
      })
      areas_v2[region] = [{
        "value": "",
        "display": "vui lòng chọn quận/huyên"
...
module.exports = {
  ...
  confOpts: {
    ...
    'bconfS': {
      cmd: 'unify-all',
      key: 'dataS',
      address: `${process.env.CCONF_HOST}/v1/internal/chapy-pro`,
