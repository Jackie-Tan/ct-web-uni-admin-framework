...
var GetData = {
  ...
  inForm: (tpl, opt, cb) => {
    ...
    Find.call(tpl, "input,textarea,select,checkbox")
      .not("[readonly]")
  ...
  inInput: (el) => {
    ...
    switch (el.attr("type")) {
      ...
      case "file":
        ...
        if (el.data("kind") === "csv") return GetData.parseCSVPromise(el);
        if (el.data("kind") === "attachment") return Images.store(files, "file");
        return Images.store(files, "image");
      default:
        if (el.data("type") == "date") {
          if (el.data("subtype") == "text") return val;
          const date = new Date(val);
          if (!date.getTime()) return null;
          if (el.data("subtype") == "number") return Math.round(date / 1000);
          return date;
        }
        if (el.data("subtype") == "number") return parseInt(val);
  ...
  compare: function (a, b) {
    ...
    if ((a == null && b !== null) || (a != null && b == null)) return b;
    if (a == null && b == null) return "_false_";
  ...
  checkDataChange: function (oldData, newData) {
    ...
    for (let key in newData) {
      ...
      if (value !== "_false_") data[key] = value;
    }
    return data;
  },
  DataManagement: Data,
